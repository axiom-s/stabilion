...
def preprocess_data(mypath):
    ...
    for i in range(len(dataX)):
        ...
        As.append(A)
        #print len(Ax), len(Ay)
        obrezka=9000 # обрезаем векторы с работой, поскольку все разной длины
        Ax=Ax[:obrezka]; Ay=Ay[:obrezka]
        Axs.append(Ax)
        Ays.append(Ay)
    ...
    return cdataX, spectrsX, cdataY, spectrsY,As, Axs, Ays
...
ill_cdataX, ill_spectrsX, ill_cdataY, ill_spectrsY, ill_As, ill_Axs, ill_Ays = preprocess_data(mypath_ill)
mypath_health='Insult_train_data/health_csv/'
health_cdataX, health_spectrsX, health_cdataY, health_spectrsY, health_As, health_Axs, health_Ays = preprocess_data(mypath_health)

#print len(ill_spectrsX), len(ill_spectrsY), len (health_spectrsX), len(health_spectrsY)



#ill_spectrs = conc(ill_spectrsX,ill_spectrsY);
#health_spectrs = conc(health_spectrsX, health_spectrsY)

ill_spectrs = conc(ill_Axs,ill_Ays); 
health_spectrs = conc(health_Axs, health_Ays)

print len(ill_spectrs[0]), len(health_spectrs[0])
